...
public class CloseTicket_Adapter extends RecyclerView.Adapter<CloseTicket_Adapter.MyViewHolderOpenTicket> {

    List<CloseTicket_Data> data;
    Context context;

    //Constructor
    public CloseTicket_Adapter(Context context, ArrayList<CloseTicket_Data> data) {
        this.context = context;
        this.data = data;
    }

    //Actualizar los datos
    public void update(List<CloseTicket_Data> list) {
        data = list;
    ...
    public void onBindViewHolder(@NonNull MyViewHolderOpenTicket holder, int position) {
        if(data != null && data.size() > 0){
            CloseTicket_Data datas = data.get(position);
